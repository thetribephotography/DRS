<x-app-layout :title="$title">
    ...
    <div class="h-[10rem] w-full bg-[#C2E7F4] pt-8 shadow">
        <div class="ml-[6rem]">
            <a href="{{ route('category.index') }}"><span
                    class="mr-2 mt-5 rounded bg-cmblue px-2.5 py-0.5 text-sm font-medium uppercase text-white">Category</span></a>
            <div class="flex">
                <h2 class="mt-2 text-s3 font-bold text-cdblack">{{ $category->name }}</h2>
                <span class="mt-4 ml-7 text-s4 font-semibold text-gg">
                    @if (count($uploads) > 0)
                        {{ count($uploads) }}
                    @endif
                </span>
    ...
    @foreach ($uploads as $upload)
        {{-- Main --}}
        <div class="mb-8 ml-[6rem] mt-7 w-full font-inter md:w-[24rem] lg:w-[60rem]">
