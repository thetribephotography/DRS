<x-app-layout :title="$title">
    ...
    <div class="h-[5rem] w-full bg-[#C2E7F4] pt-5 pb-3 shadow md:h-[10rem] md:pt-8 lg:pt-12">
        <div class="ml-6 md:ml-[6rem]">
            <h2 class="md:t-5 text-s5 font-bold text-cdblack md:text-s3">Edit Dataset</h2>
        </div>
    </div>
    {{-- End Header --}}
    ...
    <section class="my-8 h-full w-full pb-6 md:ml-[24rem] md:w-[51rem]">
        <form method="POST" action="{{ route('upload.update.dataset',$upload->id) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            {{-- Side by isde --}}
            ...
            <div class="mt-6 px-6">
                ...
                <p class="text-gray-500 mt-1 text-sm" id="file_input_help">CSV, XSLX
                    (MAX. 800x400px).</p>
                {{-- Error Mesage --}}
                @error('file-upload')
                    <div class="mt-2 rounded-lg bg-red-50 p-4 text-sm text-red-800" role="alert">
