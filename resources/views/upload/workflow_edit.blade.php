<x-app-layout :title="$title">
    ...
    <div class="h-[5rem] w-full bg-[#C2E7F4] pt-5 pb-3 shadow md:h-[10rem] md:pt-8 lg:pt-12">
        <div class="ml-6 md:ml-[6rem]">
            <h2 class="md:t-5 text-s5 font-bold text-cdblack md:text-s3">Edit Workflow Upload</h2>
        </div>
    </div>
    {{-- End Header --}}
    ...
    <section class="my-8 h-full w-full pb-6 md:ml-[24rem] md:w-[51rem]">
        <form method="POST" action="{{ route('upload.update.workflow', $upload->id) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            {{-- Side by isde --}}
